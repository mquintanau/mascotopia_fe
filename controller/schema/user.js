...
const { generateAccessToken,generateRefreshToken } = require('../auth/generateTokens'); //importamos la funcion para generar el token de acceso
...
UserSchema.pre('save', function(next){//funcion que se ejecuta antes de guardar el documento
    if(this.isNew || this.isModified('contraseña')){//si el documento es nuevo o la contraseña ha sido modificada
        const document = this;//guardamos el documento
        bcrypt.hash(document.contraseña, 10, (err, hash) => {//encriptamos la contraseña
            if(err){
...
UserSchema.methods.comparePassword = async function(contraseña, hash){//funcion para comparar la contraseña encriptada con la contraseña en texto plano
